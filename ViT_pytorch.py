...
class GSPFormer_SSI(nn.Module):
    def __init__(self, image_size, near_band, band, num_classes, dim, depth, heads, mlp_dim, pool='cls',
                 channels=1, dim_head=16, dropout=0., emb_dropout=0., mode='CAF'):
        super().__init__()
        self.name = 'GSPFormer_SSI'
        ...
        self.dropout = nn.Dropout(emb_dropout)
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, 3, mode)
    ...
    def forward(self, x, mask=None):
        ...
        x = x.squeeze(1)
        b = x.shape[0]
        x = rearrange(x, 'b h w c -> (b h w) 1 c')
        x_sorted = torch.flip(x, dims=[2])
        x_odd = x[:, :, ::2]
        x_even = x[:, :, 1::2]
        x_o_e = torch.cat((x_odd, x_even), 2)
        x_e_o = torch.cat((x_even, x_odd), 2)

        x = torch.cat((x, x_sorted, x_o_e, x_e_o), 1)
        ...
        x = self.transformer(x, mask)
        x = x[:, 0, :]
        x = x.reshape([b, self.patch_size, self.patch_size, x.shape[-1]])
        x = x.permute(0, 3, 1, 2)
        x = self.conv1(x).squeeze().squeeze()
...
if __name__ == '__main__':
    ...
    model = GSPFormer_SSI(image_size=PATCH_SIZE, near_band=band_patches, band=band, num_classes=CLASSES_NUM, dim=64,
                          depth=5, heads=4, mlp_dim=8, dropout=0.1, emb_dropout=0.1, mode=mode).cuda()
